...
import { Minus, Plus, PlusCircle, Save, Star, Trash2 } from "lucide-react";
...
export default function SurveyBuilder() {
  ...
  const [otherValues, setOtherValues] = useState({} as otherValues);
          ...
          <DialogContent>
            ...
            <div className="grid gap-4 py-4">
              ...
              <div className="flex items-center gap-4">
                ...
                <Select
                  onValueChange={(value) =>
                    setTheme(premadeThemes.find((t) => t.name === value))
                  }
                >
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(value) => {
                  ...
                  if (value === "rating" && !q.ratingScale) {
                    updateQuestion(q.id, "ratingScale", {
                      min: 1,
                      max: 5,
                      type: "number",
                    });
              ...
              {["radio", "checkbox"].includes(q.type) && (
                <div>
                  ...
                  <Select
                    value={q.layout}
                    onValueChange={(value) =>
                      updateQuestion(q.id, "layout", value)
                    }
                  >
              ...
              {["radio", "checkbox", "select"].includes(q.type) && (
                <div className="space-y-2">
                  ...
                  {q.type === "checkbox" && (
                    <div>
                      <Label htmlFor={`max-choices-${q.id}`}>
                  ...
                  <div className="flex items-center justify-between mt-4">
                    <div className="flex items-center space-x-2">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) =>
                          updateQuestion(q.id, "includeOther", checked)
                        }
                      />
                      <Label htmlFor={`include-other-${q.id}`}>
                    ...
                    <div className="flex items-center space-x-2">
                      <Checkbox
                        ...
                        checked={q.mandatory}
                        onCheckedChange={(checked) =>
                          updateQuestion(q.id, "mandatory", checked)
                        }
                      />
                      <Label htmlFor={`mandatory-${q.id}`}>
                        Mandatory question
                      </Label>
                    </div>
                  </div>
              ...
              {q.type === "rating" && (
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value) =>
                      updateRatingScale(q.id, "type", value)
                    }
                  >
                  ...
                  <div className="flex space-x-2">
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          updateRatingScale(q.id, "min", e.target.value)
                        }
                        min={1}
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          updateRatingScale(q.id, "max", e.target.value)
                        }
        ...
        <div className="max-w-md mx-auto">
          ...
          <div className="space-y-4">
              ...
              >
                {(questions[currentQuestion].options || []).map(
                  (option, index) => {
                    ...
                    const isDisabled =
                      questions[currentQuestion].maxChoices &&
                      (
                        (answers[questions[currentQuestion].id] as string[]) ||
                        []
                      ).length >= questions[currentQuestion].maxChoices &&
                      !isChecked;
                    return questions[currentQuestion].layout === "grid" ||
                      questions[currentQuestion].layout === "large-grid" ? (
                      <Card key={index}>
                        <CardContent className="flex items-center space-x-3 p-4">
                          <Checkbox
            ...
            {questions[currentQuestion].type === "rating" &&
              questions[currentQuestion].ratingScale && (
                <div>
                      ...
                      <Slider
                        value={[
                          answers[questions[currentQuestion].id] as number ||
                            questions[currentQuestion].ratingScale.min,
                        ]}
                        onValueChange={(value) =>
                          handleAnswerChange(
                            questions[currentQuestion].id,
                  ...
                  {questions[currentQuestion].ratingScale.type === "star" && (
                    <div className="flex space-x-1">
                      {Array.from(
                        { length: questions[currentQuestion].ratingScale.max },
                        (_, i) => (
                          <Star
                            key={i}
                            className={`w-6 h-6 cursor-pointer ${
                              (answers[questions[currentQuestion].id] as number || 0) > i
                ...
                <div className="flex items-center space-x-4">
                  <div className="flex items-center space-x-3">
                    <RadioGroupItem
                      value="yes"
                      id="yes"
                      className={radioStyle === "circle" ? "rounded-full" : ""}
                    />
